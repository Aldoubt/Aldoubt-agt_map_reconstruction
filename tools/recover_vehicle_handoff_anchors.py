#!/usr/bin/env python3
"""Recover vehicle-feasible handoff anchors from frozen topology anchors."""
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Read-only P1-G1.2 recovery of nearest polygon-valid handoff poses "
            "using inward-only longitudinal search at the frozen map resolution."
        )
    )
...
def main(argv=None):
    ...
    result = recover_vehicle_handoff_anchors(
        base_map,
        rectangles,
        planner_pairs,
    ...
    write_vehicle_handoff_anchor_recovery_bundle(result, output)
    ...
    print("recovered_count:", summary["recovered_count"])

    for item in result["anchors"]:
        print(
            f"{item['anchor_id']}: status={item['recovery_status']} "
            f"topology={item['topology_pose_class']} "
